...
export default function ProfileCard(props) {
  const { name, designation, phone, picture, email, socialMedia, age, _id } = props.data;
  ...
  function calculateAge(birthdate, currentDate) {
    ...
    const totalAge = currentDateObj.getFullYear() - birthdateObj.getFullYear() -
      (currentDateObj.getMonth() < birthdateObj.getMonth() ||
        (currentDateObj.getMonth() === birthdateObj.getMonth() &&
          currentDateObj.getDate() < birthdateObj.getDate()) ? 1 : 0);
  ...
  const totalAge = calculateAge(birthdate, currentDate);
  ...
  return (
    ...
    >
      <CardContent sx={{ alignItems: 'center', textAlign: 'center' }}>
        {props.children}
        <Avatar sx={{ '--Avatar-size': '4rem' }} src={`${baseURL}${picture}`} />
        <Chip
          size="sm"
          variant="soft"
          color="primary"
        ...
        <Typography level="title-lg">{name}
        </Typography>
      ...
      <CardOverflow sx={{ bgcolor: 'background.level1' }}>
        <CardActions buttonFlex="1">
          <ButtonGroup variant="outlined" sx={{ bgcolor: 'background.surface' }}>
            <Button href={"tel:" + phone} component={CustomLink} variant="outlined">Connect me</Button>
            <Button href={"mailto:" + email} component={CustomLink} target="_blank" variant="outlined">Email me</Button>
          </ButtonGroup>
        </CardActions>
      </CardOverflow>
    </Card>
